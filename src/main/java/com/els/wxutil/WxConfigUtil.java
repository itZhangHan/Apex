...
public class WxConfigUtil {
	...
	public final static String access_token_url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid="+AuthUtil.APPID+"&secret="+AuthUtil.APPSECRET;
	// 获取jsapi_ticket的接口地址（GET） 限2000（次/天）
	public final static String jsapi_ticket_url = "https://api.weixin.qq.com/cgi-bin/ticket/getticket?access_token="+AuthUtil.getAccess_token()+"&type=jsapi";
	// 缓存添加的时间
	 ...
	 * @return
	 */
	public static Map<String, Object> getSignature(String appUrl, String appId, String secret, String url,
			String urlpath) {
		// 生成签名的随机串
		String noncestr = UUID.randomUUID().toString();
		System.out.println("生成的随机串");
		if (appUrl == null || "".equals(appUrl)) {
			return null;
		}
		String signature = null;
		//根据公众号唯一APPID和唯一秘钥secret获取access_token
	...
	public static Token getToken(String appid, String appsecret, long currentTime) {
		...
		if (tockenTicketCache != null
				&& (currentTime - tockenTicketCache.getAddTime() <= tockenTicketCache.getExpiresIn())) {// 缓存存在并且没过期
			System.out.println(
					"==========缓存中token已获取时长为：" + (currentTime - tockenTicketCache.getAddTime()) + "毫秒，可以重新使用");
	...
	private static Token getTicket(String token, long currentTime) {
		...
		Token Token = null;
		if (tockenTicketCache != null
				&& (currentTime - tockenTicketCache.getAddTime() <= tockenTicketCache.getExpiresIn())) {// 缓存中有ticket
			System.out.println(
					"==========缓存中ticket已获取时长为：" + (currentTime - tockenTicketCache.getAddTime()) + "毫秒，可以重新使用");
			return tockenTicketCache;
		}
